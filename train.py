...
from utils import data_augmentation, label_rearrange,change_lab,loss_calc,cal_loss
...
for epoch in range(num_epochs):
    ...
    for batch_idx, (images, lung_lab, lesion_lab) in loop:
        ...
        with amp.autocast():
            class_out, seg_out1,seg_out2,seg_out3,seg_out4 = model(images)
            losss = cal_loss(target_seg, seg_out2, seg_out3, seg_out4)
            loss_t, loss_seg1, loss_clas, loss_seg2, loss_seg3 = loss_calc(class_out, seg_out1, target_class,
                                                                           target_seg)
        loss = 0.8 * loss_t+0.2 * losss
...
print(epoch_loss)
torch.save(model, f'model_framework21.pth')
